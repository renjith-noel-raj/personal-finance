    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-slate-900 flex items-center gap-2"><Filter size={16} /> Income Sources</h3>
          ...
          {cats.map(c => (
            <div key={c.id} className="flex items-center justify-between gap-2 p-2 border border-slate-200 rounded-lg">
              <label className="flex items-center gap-2 cursor-pointer flex-1 min-w-0">
                ...
                {c.defaultReinvest && <Repeat size={12} className="text-blue-500 flex-shrink-0" />}
              </label>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-slate-900">Income Entries</h3>
        ...
        <div className="space-y-1 max-h-96 overflow-y-auto">
          {incomes.length === 0 ? (
            <div className="text-center text-sm text-slate-400 py-8">No income for this month.</div>
              ...
              <div key={i.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-50 ${catActive[i.catId] === false ? 'opacity-40' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-900 truncate flex items-center gap-2">
                    {i.description || cat?.name}
                    {i.isLoss && <span className="px-1.5 py-0.5 rounded text-white text-xs bg-rose-500">Loss</span>}
                  </div>
                  <div className="text-xs text-slate-500 flex flex-wrap gap-2 items-center">
                    <span>{i.date}</span><span>· {cat?.name}</span>
                    {i.reinvest && <span className="text-blue-600 flex items-center gap-1"><Repeat size={10} /> {i.isLoss ? 'From capital' : 'Reinvested'}</span>}
                  </div>
                </div>
                <div className={`text-sm font-semibold ${i.isLoss ? 'text-rose-600' : 'text-emerald-600'}`}>
                  {i.isLoss ? '−' : '+'}{formatINR(i.amount)}
                </div>
                <button onClick={() => { setEditing(i); setShowForm(true); }} className="text-slate-400 hover:text-indigo-600"><Edit2 size={14} /></button>
                <button onClick={() => remove(i.id)} className="text-slate-400 hover:text-rose-600"><Trash2 size={14} /></button>
              </div>
            );
          })}
        </div>
